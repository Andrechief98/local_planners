...
namespace sfm_planner{
    ...
    bool SfmPlanner::computeVelocityCommands(geometry_msgs::Twist& cmd_vel)
    {
        ...
        if(vect_norm2(goal_coordinates,curr_robot_coordinates)<=distance_tolerance){
            ...
            theta_angle= curr_robot_orientation;
            alfa_angle = angles::normalize_angle(-theta_angle + std::atan2(new_robot_pos[1]-curr_robot_coordinates[1],new_robot_pos[0]-curr_robot_coordinates[0]));
            beta_angle = angles::normalize_angle(-theta_angle-alfa_angle);
            ...
            if(std::fabs(alfa_angle)<=_PI/2){
                ...
                //ROTAZIONE:
                new_robot_ang_vel_z=k*alfa_angle+gam*(cos(alfa_angle)*sin(alfa_angle)/alfa_angle)*(alfa_angle-beta_angle*h);
                ...
                new_robot_lin_vel={0,0};
                new_robot_ang_vel_z=sign(alfa_angle)*max_angular_vel_z;
